...
RSVM <- function(x, y, ladder, CVtype, CVnum=0 )
{
  ...
  for( i in 1:CVnum )
  {
    ...
    for( gLevel in 1:length(ladder) )
    {
      ...
      yTrain <- as.numeric(as.vector(yTrain))
      yTest <- as.numeric(as.vector(yTest))
      cat("xTrain:",str(xTrain[, SelInd]), "\n")
      cat("yTrain:",str(yTrain), "\n")
      dtrain <- xgb.DMatrix(data=xTrain[,SelInd],label=yTrain)
      dtest <- xgb.DMatrix(data=xTest[,SelInd],label=yTest)
#       
...
#       model <- xgb.train(params=params, data=dtrain,nrounds=1000)
      model <- xgboost(data=dtrain, max.depth = 5, eta = 0.01, nthread = 2, nround = 500,
                       objective = "binary:logistic", verbose = 0)
      # model <- xgboost(data=xTrain[, SelInd],label=yTrain, max.depth = 5, eta = 0.01, nthread = 2, nround = 100, 
      #                  objective = "binary:logistic", verbose = 0)
      

      if( CVtype == "LOO" )
      {
        svmpred <- predict(model,dtest)
        svmpred.train <- predict(model,dtrain)
      } else
      {
        svmpred <- predict(model,dtest)
        svmpred.train <- predict(model,dtrain)
...
#       cat("yTest",yTest,"\n")
      ...
      svmpred <- as.numeric(svmpred > 0.5)
      svmpred.train <- as.numeric(svmpred.train > 0.5)
      ...
      ## weight vector
      Wall <- rep(0, length(SelInd))
      names(Wall) <- colnames(xTrain[, SelInd])
      W <- xgb.importance( colnames(xTrain[, SelInd]),model = model)
      save(model,file = "model.Rdata")
      cat("W:", str(W) ,"\n")
      Wall[W$Feature] <- W$Frequence
      rkW <- rank(Wall)
      cat("Wall:", str(Wall) ,"\n")
      ## record the genes selected in this ladder
      SelFreq[names(Wall), gLevel] <- SelFreq[names(Wall), gLevel] + Wall
...
SummaryRSVM <- function( RSVMres )
{
  ...
  # bootstrap
  ERInd.boot <- max( which(RSVMres$bootError == min(RSVMres$bootError)) )
  ...
  ret <- list( MinER=min(RSVMres$Error), MinLevel=MinLevel, SelInd=SelInd,bootstrapInfo=ret.boot,
               Errorlist_test=RSVMres$Error.list, Errorlist_train=RSVMres$Error_train.list,RSVMres=RSVMres)
  return(ret)
}
